...
export async function generateMetadata({ params }: { params: { id: string } }): Promise<Metadata> {
  try {
    ...
    const title   = `${d.caratWeight}ct ${d.shape.charAt(0).toUpperCase() + d.shape.slice(1)} Diamond — ${d.cut} Cut`;
    const ogTitle = `${title} | Sterling Jewellers`;
    return {
      title,
      ...
      openGraph: { title: ogTitle, description: `${d.caratWeight}ct ${d.shape} ${d.color}/${d.clarity}/${d.cut}`, images: d.imageUrl ? [d.imageUrl] : [] },
    };
  } catch {
    return { title: 'Diamond Details' };
  }
}
